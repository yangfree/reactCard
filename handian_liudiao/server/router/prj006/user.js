...
router.post('/search', function (req, res, next) {
  ...
  const options = {
    url: myConst.apiurl + "/prj006/search/?page=" + req.body.page,
...
router.post('/export', function (req, res, next) {
  const options = {
    url: myConst.apiurl + "/prj006/export/",
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj006token.access_token }
  }
...
router.post('/add', function (req, res, next) {
  ...
  const options = {
    url: myConst.apiurl + "/prj006/info/",
    form: req.body,
...
router.delete('/remove', function (req, res, next) {
  ...
  let { _id } = req.body;
  const options = {
    url: myConst.apiurl + "/prj006/info/" + _id,
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj006token.access_token }
  }
...
router.post('/check', function (req, res, next) {
  ...
  //   reason_for_check: req.body.reason_for_check,
  //   headers: {
  //     'Authorization': 'Bearer ' + req.cookies.prj006token.access_token
  //   }
  // }
  // console.log("审核", options)

  $http({
    url: `${myConst.apiurl}/prj006/info/check/${_id}`,
    method: "POST",
    data: {
...
router.post('/list', function (req, res, next) {
  ...
  request.get(options, function (error, response, body) {
    ...
    var is_admin = bodyParse.is_admin // 判断是PC还是mobile
    console.log('prj006返回的数据:', bodyParse);
...
router.get('/form', function (req, res, next) {
  ...
  let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj006token;
  var options = {
    url: `${myConst.apiurl}/prj006/${formName}/${_id}`,
    // url: myConst.apiurl + '/prj006/'+req.query.formName + '/' + req.query._id,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
...
router.put('/form', function (req, res, next) {
  ...
  let options = {
    url: `${myConst.apiurl}/prj006/${formName}/${_id}`,
    // url: myConst.apiurl + '/prj006/info/' + req.body._id + '/',
    infoForm: infoForm,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
...
router.post('/hide', function (req, res, next) {
  ...
  let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj006token;
  let options = {
    url: myConst.apiurl + "/prj006/hide/",
    form: hideParams,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
