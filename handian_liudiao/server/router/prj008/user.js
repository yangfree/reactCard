...
router.post('/search', function (req, res, next) {
  ...
  const options = {
    url: myConst.apiurl + "/prj008/search/?page=" + req.body.page,
    form: req.body,//浏览器发送过来的req的body  和  后端返回的response的body格式类型不一样?
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj008token.access_token }
  }
...
router.post('/export', function (req, res, next) {
  const options = {
    url: myConst.apiurl + "/prj008/export/",
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj008token.access_token }
  }
...
router.post('/add', function (req, res, next) {
  ...
  let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj008token;
  const options = {
    url: myConst.apiurl + "/prj008/info/",
    form: req.body,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
  console.log('新建请求参数->', options)
  request.post(options, function (error, response, body) {
    console.log("增加信息08", body)
...
router.delete('/remove', function (req, res, next) {
  ...
  let { _id } = req.body;
  const options = {
    url: myConst.apiurl + "/prj008/info/" + _id,
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj008token.access_token }
  }
...
router.post('/check', function (req, res, next) {
  ...
  $http({
    url: `${myConst.apiurl}/prj008/info/check/${_id}`,
    method: "POST",
    data: {
    ...
    headers: {
      'Authorization': 'Bearer ' + req.cookies.prj008token.access_token
    }
  })
    .then((response) => {
...
router.post('/list', function (req, res, next) {
  ...
  request.get(options, function (error, response, body) {
    ...
    console.log('prj008返回的数据:', bodyParse);
...
router.get('/form', function (req, res, next) {
  ...
  let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj008token;
  var options = {
    url: `${myConst.apiurl}/prj008/${formName}/${_id}`,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
...
//   let options = {
//     url: myConst.apiurl + "/prj008/hide/",
//     form: hideParams,
//     headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
//   }
