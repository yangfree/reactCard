...
router.post('/search', function (req, res, next) {
  ...
  const options = {
    url: myConst.apiurl + "/prj00g/search/?page=" + req.body.page,
    form: req.body,//浏览器发送过来的req的body  和  后端返回的response的body格式类型不一样?
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj00gtoken.access_token }
  }
...
router.post('/export', function (req, res, next) {
  const options = {
    url: myConst.apiurl + "/prj00g/export/",
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj00gtoken.access_token }
  }
...
router.post('/add', function (req, res, next) {
  ...
  let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj00gtoken;
  const options = {
    url: myConst.apiurl + "/prj00g/info/",
    form: req.body,
...
router.delete('/remove', function (req, res, next) {
  ...
  let { _id } = req.body;
  const options = {
    url: myConst.apiurl + "/prj00g/info/" + _id,
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj00gtoken.access_token }
  }
...
router.post('/check', function (req, res, next) {
  ...
  $http({
    url: `${myConst.apiurl}/prj00g/info/check/${_id}`,
    method: "POST",
    data: {
    ...
    headers: {
      'Authorization': 'Bearer ' + req.cookies.prj00gtoken.access_token
    }
  })
    .then((response) => {
...
router.post('/list', function (req, res, next) {
  ...
  request.get(options, function (error, response, body) {
    ...
    console.log('prj00g返回的数据:', bodyParse);
...
router.get('/form', function (req, res, next) {
  ...
  let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj00gtoken;
  var options = {
    url: `${myConst.apiurl}/prj00g/${formName}/${_id}`,
    // url: myConst.apiurl + '/prj00g/'+req.query.formName + '/' + req.query._id,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
...
router.put('/form', function (req, res, next) {
  ...
  let options = {
    url: `${myConst.apiurl}/prj00g/${formName}/${_id}`,
    // url: myConst.apiurl + '/prj00g/info/' + req.body._id + '/',
    infoForm: infoForm,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
...
//   let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj00gtoken;
//   let options = {
//     url: myConst.apiurl + "/prj00g/hide/",
//     form: hideParams,
//     headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
//   }
