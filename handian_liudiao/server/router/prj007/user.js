...
//   const options = {
//     url: myConst.apiurl + "/prj007/search/?page=" + req.body.page,
//     form: req.body,//浏览器发送过来的req的body  和  后端返回的response的body格式类型不一样?
//     headers: { 'Authorization': 'Bearer ' + req.cookies.prj006token.access_token }
//   }
...
router.post('/add', function (req, res, next) {
  ...
  let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj007token;
  const options = {
    url: myConst.apiurl + "/prj007/info/",
    form: req.body,
...
router.delete('/remove', function (req, res, next) {
  ...
  let { _id } = req.body;
  const options = {
    url: myConst.apiurl + "/prj007/info/" + _id,
    headers: { 'Authorization': 'Bearer ' + req.cookies.prj007token.access_token }
  }
...
router.post('/check', function (req, res, next) {
  ...
  $http({
    url: `${myConst.apiurl}/prj007/info/check/${_id}`,
    method: "POST",
    data: {
    ...
    headers: {
      'Authorization': 'Bearer ' + req.cookies.prj007token.access_token
    }
  })
    .then((response) => {
...
router.post('/list', function (req, res, next) {
  ...
  request.get(options, function (error, response, body) {
    ...
    console.log('prj007返回的数据:', bodyParse);
...
//   let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj006token;
//   var options = {
//     url: `${myConst.apiurl}/prj007/${formName}/${_id}`,
//     // url: myConst.apiurl + '/prj007/'+req.query.formName + '/' + req.query._id,
//     headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
//   }
...
router.put('/form', function (req, res, next) {
  ...
  let options = {
    url: `${myConst.apiurl}/prj007/${formName}/${_id}`,
    // url: myConst.apiurl + '/prj007/info/' + req.body._id + '/',
    infoForm: infoForm,
    headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
  }
...
//   let myOptions = req.cookies.mobile_token ? req.cookies.mobile_token : req.cookies.prj006token;
//   let options = {
//     url: myConst.apiurl + "/prj007/hide/",
//     form: hideParams,
//     headers: { 'Authorization': 'Bearer ' + myOptions.access_token }
//   }
